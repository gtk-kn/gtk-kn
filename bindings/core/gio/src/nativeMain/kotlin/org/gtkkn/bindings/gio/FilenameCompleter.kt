...
public open class FilenameCompleter(
     ...
     *     it when finished.
     */
    public open fun getCompletionSuffix(initialText: String): String? = g_filename_completer_get_completion_suffix(gioFilenameCompleterPointer.reinterpret(), initialText)?.toKString()
     ...
     */
    public open fun getCompletions(initialText: String): List<String> = g_filename_completer_get_completions(gioFilenameCompleterPointer.reinterpret(), initialText)?.toKStringList() ?: error("Expected not null string array")
     ...
     */
    public open fun setDirsOnly(dirsOnly: Boolean): Unit = g_filename_completer_set_dirs_only(gioFilenameCompleterPointer.reinterpret(), dirsOnly.asGBoolean())
     ...
     */
    public fun connectGotCompletionData(connectFlags: ConnectFlags = ConnectFlags(0u), handler: () -> Unit): ULong = g_signal_connect_data(gPointer.reinterpret(), "got-completion-data", connectGotCompletionDataFunc.reinterpret(), StableRef.create(handler).asCPointer(), staticStableRefDestroy.reinterpret(), connectFlags.mask)
