...
public open class FileInputStream(
    ...
    ): Unit = g_file_input_stream_query_info_async(gioFileInputStreamPointer.reinterpret(), attributes, ioPriority, cancellable?.gioCancellablePointer?.reinterpret(), callback?.let { AsyncReadyCallbackFunc.reinterpret() }, callback?.let { StableRef.create(callback).asCPointer() })
