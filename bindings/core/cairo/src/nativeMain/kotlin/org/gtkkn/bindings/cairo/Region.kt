...
@CairoVersion1_10
public class Region(
    public val cairoRegionPointer: CPointer<cairo_region_t>,
) : ProxyInstance(cairoRegionPointer) {
    public constructor() : this(cairo_region_create()!!) {
        MemoryCleaner.setFreeFunc(this, owned = true) { cairo_region_destroy(it.reinterpret()) }
    }

    public constructor(rectangle: RectangleInt) : this(cairo_region_create_rectangle(rectangle.cairoRectangleIntPointer)!!) {
        MemoryCleaner.setFreeFunc(this, owned = true) { cairo_region_destroy(it.reinterpret()) }
    }
    ...
    public fun getExtents(extents: RectangleInt): Unit = cairo_region_get_extents(cairoRegionPointer, extents.cairoRectangleIntPointer)

    public fun numRectangles(): gint = cairo_region_num_rectangles(cairoRegionPointer)

    public fun getRectangle(nth: gint, rectangle: RectangleInt): Unit = cairo_region_get_rectangle(cairoRegionPointer, nth, rectangle.cairoRectangleIntPointer)

    public fun isEmpty(): Boolean = cairo_region_is_empty(cairoRegionPointer).asBoolean()

    public fun containsPoint(x: gint, y: gint): Boolean = cairo_region_contains_point(cairoRegionPointer, x, y).asBoolean()

    public fun containsRectangle(rectangle: RectangleInt): RegionOverlap = cairo_region_contains_rectangle(cairoRegionPointer, rectangle.cairoRectangleIntPointer).run {
        RegionOverlap.fromNativeValue(this)}

    public fun equal(other: Region): Boolean = cairo_region_equal(cairoRegionPointer, other.cairoRegionPointer).asBoolean()

    public fun translate(dx: gint, dy: gint): Unit = cairo_region_translate(cairoRegionPointer, dx, dy)

    public fun intersect(other: Region): Status = cairo_region_intersect(cairoRegionPointer, other.cairoRegionPointer).run {
        Status.fromNativeValue(this)}

    public fun intersectRectangle(rectangle: RectangleInt): Status = cairo_region_intersect_rectangle(cairoRegionPointer, rectangle.cairoRectangleIntPointer).run {
        Status.fromNativeValue(this)}

    public fun subtract(other: Region): Status = cairo_region_subtract(cairoRegionPointer, other.cairoRegionPointer).run {
        Status.fromNativeValue(this)}

    public fun subtractRectangle(rectangle: RectangleInt): Status = cairo_region_subtract_rectangle(cairoRegionPointer, rectangle.cairoRectangleIntPointer).run {
        Status.fromNativeValue(this)}

    public fun union(other: Region): Status = cairo_region_union(cairoRegionPointer, other.cairoRegionPointer).run {
        Status.fromNativeValue(this)}

    public fun unionRectangle(rectangle: RectangleInt): Status = cairo_region_union_rectangle(cairoRegionPointer, rectangle.cairoRectangleIntPointer).run {
        Status.fromNativeValue(this)}

    public fun xor(other: Region): Status = cairo_region_xor(cairoRegionPointer, other.cairoRegionPointer).run {
        Status.fromNativeValue(this)}

    public fun xorRectangle(rectangle: RectangleInt): Status = cairo_region_xor_rectangle(cairoRegionPointer, rectangle.cairoRectangleIntPointer).run {
        Status.fromNativeValue(this)}
