...
public class Value(
    ...
    ) : this(jsc_value_new_array_buffer(context.javascriptcoreContextPointer.reinterpret(), `data`, size, destroyNotify?.let { DestroyNotifyFunc.reinterpret() }, destroyNotify?.let { StableRef.create(destroyNotify).asCPointer() })!!.reinterpret())
    ...
    public constructor(
        ...
        jscClass: Class? = null,
    ) : this(jsc_value_new_object(context.javascriptcoreContextPointer.reinterpret(), instance, jscClass?.javascriptcoreClassPointer?.reinterpret())!!.reinterpret())
