...
public class Context(
    ...
    public val virtualMachine: VirtualMachine
        ...
        get() = jsc_context_get_virtual_machine(jscContextPointer)!!.run {
            InstanceCache.get(this, true) { VirtualMachine(reinterpret()) }!!.apply { ref() }
        }
     ...
     */
    public fun evaluate(code: String, length: Long): Value = jsc_context_evaluate(jscContextPointer, code, length)!!.run {
    ...
    public fun evaluateWithSourceUri(
        ...
        uri: String,
        lineNumber: guint,
    ): Value = jsc_context_evaluate_with_source_uri(jscContextPointer, code, length, uri, lineNumber)!!.run {
        InstanceCache.get(this, true) { Value(reinterpret()) }!!.apply { ref() }
    }
    ...
    public fun getException(): Exception? = jsc_context_get_exception(jscContextPointer)?.run {
        InstanceCache.get(this, true) { Exception(reinterpret()) }!!.apply { ref() }
    }
     ...
     */
    public fun pushExceptionHandler(handler: ExceptionHandler): Unit = jsc_context_push_exception_handler(jscContextPointer, ExceptionHandlerFunc.reinterpret(), StableRef.create(handler).asCPointer(), staticStableRefDestroy.reinterpret())
     ...
     */
    public fun setValue(name: String, `value`: Value): Unit = jsc_context_set_value(jscContextPointer, name, `value`.jscValuePointer)
     ...
     */
    public fun throwException(exception: Exception): Unit = jsc_context_throw_exception(jscContextPointer, exception.jscExceptionPointer)
     ...
     * @param errorMessage an error message
     */
    public fun throwWithName(errorName: String, errorMessage: String): Unit = jsc_context_throw_with_name(jscContextPointer, errorName, errorMessage)
