...
public class Context(pointer: CPointer<JSCContext>) :
    ...
    public val virtualMachine: VirtualMachine
         ...
         */
        get() = jsc_context_get_virtual_machine(jscContextPointer.reinterpret())!!.run {
    ...
    public constructor(
        vm: VirtualMachine,
    ) : this(jsc_context_new_with_virtual_machine(vm.jscVirtualMachinePointer.reinterpret())!!.reinterpret())

    /**
     * Clear the uncaught exception in @context if any.
     */
    public fun clearException(): Unit = jsc_context_clear_exception(jscContextPointer.reinterpret())
    ...
    public fun evaluate(code: String, length: Long): Value =
        jsc_context_evaluate(jscContextPointer.reinterpret(), code, length)!!.run {
            Value(reinterpret())
        }
    ...
    public fun evaluateWithSourceUri(code: String, length: Long, uri: String, lineNumber: guint): Value =
        jsc_context_evaluate_with_source_uri(jscContextPointer.reinterpret(), code, length, uri, lineNumber)!!.run {
            Value(reinterpret())
        }
     ...
     */
    public fun getException(): Exception? = jsc_context_get_exception(jscContextPointer.reinterpret())?.run {
     ...
     * @return a #JSCValue
     */
    public fun getGlobalObject(): Value = jsc_context_get_global_object(jscContextPointer.reinterpret())!!.run {
        Value(reinterpret())
    }
     ...
     * @return a #JSCValue
     */
    public fun getValue(name: String): Value = jsc_context_get_value(jscContextPointer.reinterpret(), name)!!.run {
        Value(reinterpret())
    }
     ...
     */
    public fun popExceptionHandler(): Unit = jsc_context_pop_exception_handler(jscContextPointer.reinterpret())
    ...
    public fun setValue(name: String, `value`: Value): Unit =
        jsc_context_set_value(jscContextPointer.reinterpret(), name, `value`.jscValuePointer.reinterpret())
     ...
     */
    public fun `throw`(errorMessage: String): Unit = jsc_context_throw(jscContextPointer.reinterpret(), errorMessage)
    ...
    public fun throwException(exception: Exception): Unit =
        jsc_context_throw_exception(jscContextPointer.reinterpret(), exception.jscExceptionPointer.reinterpret())
    ...
    public fun throwWithName(errorName: String, errorMessage: String): Unit =
        jsc_context_throw_with_name(jscContextPointer.reinterpret(), errorName, errorMessage)
