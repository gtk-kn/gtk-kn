...
public interface HoverProvider : Interface, KGTyped {
    ...
    ): Unit = gtk_source_hover_provider_populate_async(gtksourceHoverProviderPointer.reinterpret(), context.gtksourceHoverContextPointer.reinterpret(), display.gtksourceHoverDisplayPointer.reinterpret(), cancellable?.gioCancellablePointer?.reinterpret(), callback?.let { AsyncReadyCallbackFunc.reinterpret() }, callback?.let { StableRef.create(callback).asCPointer() })
