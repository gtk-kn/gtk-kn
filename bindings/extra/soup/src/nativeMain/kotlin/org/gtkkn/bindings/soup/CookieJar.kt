...
public open class CookieJar(
     ...
     */
    public open fun addCookie(cookie: Cookie): Unit = soup_cookie_jar_add_cookie(soupCookieJarPointer, cookie.soupCookiePointer)
    ...
    public open fun addCookieFull(
        ...
        uri: Uri? = null,
        firstParty: Uri? = null,
    ): Unit = soup_cookie_jar_add_cookie_full(soupCookieJarPointer, cookie.soupCookiePointer, uri?.glibUriPointer, firstParty?.glibUriPointer)
     ...
     * @param cookie a #SoupCookie
     */
    public open fun addCookieWithFirstParty(firstParty: Uri, cookie: Cookie): Unit = soup_cookie_jar_add_cookie_with_first_party(soupCookieJarPointer, firstParty.glibUriPointer, cookie.soupCookiePointer)
     ...
     * @param cookie a #SoupCookie
     */
    public open fun deleteCookie(cookie: Cookie): Unit = soup_cookie_jar_delete_cookie(soupCookieJarPointer, cookie.soupCookiePointer)
     ...
     */
    public open fun getCookieList(uri: Uri, forHttp: Boolean): SList = soup_cookie_jar_get_cookie_list(soupCookieJarPointer, uri.glibUriPointer, forHttp.asGBoolean())!!.run {
        SList(this)}
    ...
    ): SList = soup_cookie_jar_get_cookie_list_with_same_site_info(soupCookieJarPointer, uri.glibUriPointer, topLevel?.glibUriPointer, siteForCookies?.glibUriPointer, forHttp.asGBoolean(), isSafeMethod.asGBoolean(), isTopLevelNavigation.asGBoolean())!!.run {
        SList(this)}
     ...
     */
    public open fun getCookies(uri: Uri, forHttp: Boolean): String? = soup_cookie_jar_get_cookies(soupCookieJarPointer, uri.glibUriPointer, forHttp.asGBoolean())?.toKString()
     ...
     */
    public open fun setCookie(uri: Uri, cookie: String): Unit = soup_cookie_jar_set_cookie(soupCookieJarPointer, uri.glibUriPointer, cookie)
    ...
    public open fun setCookieWithFirstParty(
        ...
        firstParty: Uri,
        cookie: String,
    ): Unit = soup_cookie_jar_set_cookie_with_first_party(soupCookieJarPointer, uri.glibUriPointer, firstParty.glibUriPointer, cookie)
     ...
     */
    public fun onChanged(connectFlags: ConnectFlags = ConnectFlags(0u), handler: (oldCookie: Cookie, newCookie: Cookie) -> Unit): ULong = g_signal_connect_data(soupCookieJarPointer, "changed", onChangedFunc.reinterpret(), StableRef.create(handler).asCPointer(), staticStableRefDestroy.reinterpret(), connectFlags.mask)
    ...
    public fun emitChanged(oldCookie: Cookie, newCookie: Cookie) {
        g_signal_emit_by_name(soupCookieJarPointer.reinterpret(), "changed", oldCookie.soupCookiePointer, newCookie.soupCookiePointer)
    }
    ...
    public companion object : TypeCompanion<CookieJar> {
         ...
         */
        internal fun getTypeOrNull(): GType? = org.gtkkn.extensions.glib.cinterop.getTypeOrNull("soup_cookie_jar_get_type")
    }
}
