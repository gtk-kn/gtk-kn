...
public class WebInspector(
    ...
    public val inspectedUri: String
         ...
         */
        get() = webkit_web_inspector_get_inspected_uri(webkitWebInspectorPointer)?.toKString() ?: error("Expected not null string")
    ...
    public fun getWebView(): WebViewBase = webkit_web_inspector_get_web_view(webkitWebInspectorPointer)!!.run {
        InstanceCache.get(this, true) { WebViewBase(reinterpret()) }!!.apply { ref() }
    }
     ...
     */
    public fun onAttach(connectFlags: ConnectFlags = ConnectFlags(0u), handler: () -> Boolean): ULong = g_signal_connect_data(webkitWebInspectorPointer, "attach", onAttachFunc.reinterpret(), StableRef.create(handler).asCPointer(), staticStableRefDestroy.reinterpret(), connectFlags.mask)
     ...
     *    false to propagate the event further.
     */
    public fun onBringToFront(connectFlags: ConnectFlags = ConnectFlags(0u), handler: () -> Boolean): ULong = g_signal_connect_data(webkitWebInspectorPointer, "bring-to-front", onBringToFrontFunc.reinterpret(), StableRef.create(handler).asCPointer(), staticStableRefDestroy.reinterpret(), connectFlags.mask)
     ...
     */
    public fun onClosed(connectFlags: ConnectFlags = ConnectFlags(0u), handler: () -> Unit): ULong = g_signal_connect_data(webkitWebInspectorPointer, "closed", onClosedFunc.reinterpret(), StableRef.create(handler).asCPointer(), staticStableRefDestroy.reinterpret(), connectFlags.mask)
     ...
     *    false to propagate the event further.
     */
    public fun onDetach(connectFlags: ConnectFlags = ConnectFlags(0u), handler: () -> Boolean): ULong = g_signal_connect_data(webkitWebInspectorPointer, "detach", onDetachFunc.reinterpret(), StableRef.create(handler).asCPointer(), staticStableRefDestroy.reinterpret(), connectFlags.mask)
     ...
     *    false to propagate the event further.
     */
    public fun onOpenWindow(connectFlags: ConnectFlags = ConnectFlags(0u), handler: () -> Boolean): ULong = g_signal_connect_data(webkitWebInspectorPointer, "open-window", onOpenWindowFunc.reinterpret(), StableRef.create(handler).asCPointer(), staticStableRefDestroy.reinterpret(), connectFlags.mask)

    public companion object : TypeCompanion<WebInspector> {
        override val type: GeneratedClassKGType<WebInspector> =
                GeneratedClassKGType(getTypeOrNull()!!) { WebInspector(it.reinterpret()) }
        ...
        /**
         * Gets the GType of from the symbol `webkit_web_inspector_get_type` if it exists.
         *
         ...
         */
        internal fun getTypeOrNull(): GType? = org.gtkkn.extensions.glib.cinterop.getTypeOrNull("webkit_web_inspector_get_type")
    }
}
