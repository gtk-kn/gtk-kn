...
public open class Filter(
     ...
     */
    public open fun match(item: Object): Boolean = gtk_filter_match(gtkFilterPointer, item.gobjectObjectPointer.reinterpret()).asBoolean()
     ...
     */
    public fun onChanged(connectFlags: ConnectFlags = ConnectFlags(0u), handler: (change: FilterChange) -> Unit): ULong = g_signal_connect_data(gtkFilterPointer, "changed", onChangedFunc.reinterpret(), StableRef.create(handler).asCPointer(), staticStableRefDestroy.reinterpret(), connectFlags.mask)
