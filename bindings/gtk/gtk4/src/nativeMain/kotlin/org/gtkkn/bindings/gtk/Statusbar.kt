...
public open class Statusbar(
     ...
     */
    public open fun getContextId(contextDescription: String): guint = gtk_statusbar_get_context_id(gtkStatusbarPointer.reinterpret(), contextDescription)
     ...
     */
    public open fun push(contextId: guint, text: String): guint = gtk_statusbar_push(gtkStatusbarPointer.reinterpret(), contextId, text)
     ...
     */
    public open fun remove(contextId: guint, messageId: guint): Unit = gtk_statusbar_remove(gtkStatusbarPointer.reinterpret(), contextId, messageId)
     ...
     * @param contextId a context identifier
     */
    public open fun removeAll(contextId: guint): Unit = gtk_statusbar_remove_all(gtkStatusbarPointer.reinterpret(), contextId)
     ...
     */
    public fun connectTextPopped(connectFlags: ConnectFlags = ConnectFlags(0u), handler: (contextId: guint, text: String) -> Unit): ULong = g_signal_connect_data(gPointer.reinterpret(), "text-popped", connectTextPoppedFunc.reinterpret(), StableRef.create(handler).asCPointer(), staticStableRefDestroy.reinterpret(), connectFlags.mask)
     ...
     */
    public fun connectTextPushed(connectFlags: ConnectFlags = ConnectFlags(0u), handler: (contextId: guint, text: String) -> Unit): ULong = g_signal_connect_data(gPointer.reinterpret(), "text-pushed", connectTextPushedFunc.reinterpret(), StableRef.create(handler).asCPointer(), staticStableRefDestroy.reinterpret(), connectFlags.mask)
...
private val connectTextPoppedFunc: CPointer<CFunction<(guint, CPointer<ByteVar>) -> Unit>> =
    ...
    ->
    userData.asStableRef<(contextId: guint, text: String) -> Unit>().get().invoke(contextId, text?.toKString() ?: error("Expected not null string"))}
.reinterpret()
