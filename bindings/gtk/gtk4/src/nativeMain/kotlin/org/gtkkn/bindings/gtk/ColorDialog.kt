...
public open class ColorDialog(
    ...
    ): Unit = gtk_color_dialog_choose_rgba(gtkColorDialogPointer, parent?.gtkWindowPointer, initialColor?.gdkRgbaPointer, cancellable?.gioCancellablePointer, callback?.let { AsyncReadyCallbackFunc.reinterpret() }, callback?.let { StableRef.create(callback).asCPointer() })
    ...
    public open fun chooseRgbaFinish(result: AsyncResult): Result<Rgba?> = memScoped {
        ...
        val gResult = gtk_color_dialog_choose_rgba_finish(gtkColorDialogPointer, result.gioAsyncResultPointer, gError.ptr)?.run {
            Rgba(this)}
    ...
    public companion object : TypeCompanion<ColorDialog> {
         ...
         */
        internal fun getTypeOrNull(): GType? = org.gtkkn.extensions.glib.cinterop.getTypeOrNull("gtk_color_dialog_get_type")
    }
}
