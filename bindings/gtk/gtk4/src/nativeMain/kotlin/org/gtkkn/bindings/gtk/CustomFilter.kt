...
public open class CustomFilter(
     ...
     */
    public constructor(matchFunc: CustomFilterFunc?) : this(gtk_custom_filter_new(matchFunc?.let { CustomFilterFuncFunc.reinterpret() }, matchFunc?.let { StableRef.create(matchFunc).asCPointer() }, matchFunc?.let { staticStableRefDestroy.reinterpret() })!!) {
     ...
     * @param matchFunc function to filter items
     */
    public open fun setFilterFunc(matchFunc: CustomFilterFunc?): Unit = gtk_custom_filter_set_filter_func(gtkCustomFilterPointer, matchFunc?.let { CustomFilterFuncFunc.reinterpret() }, matchFunc?.let { StableRef.create(matchFunc).asCPointer() }, matchFunc?.let { staticStableRefDestroy.reinterpret() })
    ...
    public companion object : TypeCompanion<CustomFilter> {
         ...
         */
        internal fun getTypeOrNull(): GType? = org.gtkkn.extensions.glib.cinterop.getTypeOrNull("gtk_custom_filter_get_type")
    }
}
