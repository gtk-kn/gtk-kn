 ...
 */
public open class GestureClick(
     ...
     */
    public fun onPressed(connectFlags: ConnectFlags = ConnectFlags(0u), handler: (
        nPress: gint,
        x: gdouble,
        y: gdouble,
    ) -> Unit): ULong = g_signal_connect_data(gtkGestureClickPointer, "pressed", onPressedFunc.reinterpret(), StableRef.create(handler).asCPointer(), staticStableRefDestroy.reinterpret(), connectFlags.mask)
     ...
     */
    public fun onReleased(connectFlags: ConnectFlags = ConnectFlags(0u), handler: (
        nPress: gint,
        x: gdouble,
        y: gdouble,
    ) -> Unit): ULong = g_signal_connect_data(gtkGestureClickPointer, "released", onReleasedFunc.reinterpret(), StableRef.create(handler).asCPointer(), staticStableRefDestroy.reinterpret(), connectFlags.mask)
     ...
     */
    public fun onStopped(connectFlags: ConnectFlags = ConnectFlags(0u), handler: () -> Unit): ULong = g_signal_connect_data(gtkGestureClickPointer, "stopped", onStoppedFunc.reinterpret(), StableRef.create(handler).asCPointer(), staticStableRefDestroy.reinterpret(), connectFlags.mask)
     ...
     */
    public fun onUnpairedRelease(connectFlags: ConnectFlags = ConnectFlags(0u), handler: (
        x: gdouble,
        y: gdouble,
        button: guint,
        sequence: EventSequence?,
    ) -> Unit): ULong = g_signal_connect_data(gtkGestureClickPointer, "unpaired-release", onUnpairedReleaseFunc.reinterpret(), StableRef.create(handler).asCPointer(), staticStableRefDestroy.reinterpret(), connectFlags.mask)
     ...
     */
    public fun emitUnpairedRelease(
        x: gdouble,
        y: gdouble,
        button: guint,
        sequence: EventSequence?,
    ) {
        g_signal_emit_by_name(gtkGestureClickPointer.reinterpret(), "unpaired-release", x, y, button, sequence?.gdkEventSequencePointer)
    }

    public companion object : TypeCompanion<GestureClick> {
        override val type: GeneratedClassKGType<GestureClick> =
                GeneratedClassKGType(getTypeOrNull()!!) { GestureClick(it.reinterpret()) }
        ...
        /**
         * Gets the GType of from the symbol `gtk_gesture_click_get_type` if it exists.
         *
         ...
         */
        internal fun getTypeOrNull(): GType? = org.gtkkn.extensions.glib.cinterop.getTypeOrNull("gtk_gesture_click_get_type")
    }
}

private val onPressedFunc: CPointer<CFunction<(
    gint,
    gdouble,
    gdouble,
    ...
    userData.asStableRef<(
        ...
        x: gdouble,
        y: gdouble,
    ) -> Unit>().get().invoke(nPress, x, y)}
.reinterpret()

private val onReleasedFunc: CPointer<CFunction<(
    gint,
    gdouble,
    gdouble,
) -> Unit>> = staticCFunction {
...
private val onUnpairedReleaseFunc: CPointer<CFunction<(
    gdouble,
    gdouble,
    guint,
    ...
    userData.asStableRef<(
        ...
        button: guint,
        sequence: EventSequence?,
    ) -> Unit>().get().invoke(x, y, button, sequence?.run {
        EventSequence(this)}
    )}
.reinterpret()
