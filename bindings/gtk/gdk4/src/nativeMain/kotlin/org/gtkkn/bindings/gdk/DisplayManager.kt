...
public open class DisplayManager(
     ...
     */
    public open fun setDefaultDisplay(display: Display): Unit = gdk_display_manager_set_default_display(gdkDisplayManagerPointer, display.gdkDisplayPointer)
     ...
     */
    public fun onDisplayOpened(connectFlags: ConnectFlags = ConnectFlags(0u), handler: (display: Display) -> Unit): ULong = g_signal_connect_data(gdkDisplayManagerPointer, "display-opened", onDisplayOpenedFunc.reinterpret(), StableRef.create(handler).asCPointer(), staticStableRefDestroy.reinterpret(), connectFlags.mask)
    ...
    public companion object : TypeCompanion<DisplayManager> {
         ...
         */
        internal fun getTypeOrNull(): GType? = org.gtkkn.extensions.glib.cinterop.getTypeOrNull("gdk_display_manager_get_type")
    }
}
